<!-- <form action="/payment/checkout" method="post" onsubmit="checkForm(retreat)"> -->
<form action="/process-transaction" method="post" onsubmit="checkForm(retreat)">
    {!! csrf_field() !!}
    ...
    <div class="modal-body p-0">
        <div class="row gx-0">
            <div class="col-md">
                <div class="h-100 p-4 ">
                    <div>
                        <h4 class="mb-0">{{ $retreat->name }} @ {{ $venue->name }}</h4>
                        <small class="text-muted">
                            <i class="fas fa-calendar me-2"></i>
                            {{ \Carbon\Carbon::parse(12/12/2023)->format('d-M-Y | h:i A') }} Onwards
                        </small>
                    </div>
                    <div class="table-responsive-sm">
                        <table class="table table-ms mt-3">
                            @foreach ($bookings as $n => $retreat_booking)
                            <tr data-row="ticket">
                                <td width="50%" class="ps-0">
                                    <h6 class="mb-0 ">{{ $retreat_booking->name }}
                                        <br />
                                        <span class="badge bg-danger">
                                            {{ \Carbon\Carbon::parse($retreat_booking->start_datetime)->format('d-m-Y') }}
                                        </span>
                                    </h6>
                                    <small class="text-muted">{{$retreat_booking->description}}</small><br />
                                    <span class=""> @money($retreat_booking->price)</span>
                                </td>
                                <td class="align-middle pe-0 pr-0" width="1%">
                                    <input type="hidden" name="items[{{ $n }}][retreat_booking_id]" value="{{ $retreat_booking->id }}" class="" />
                                    <input type="hidden" value="{{ $retreat_booking->price }}" data-field="price" />
                                    <div class="input-group" style="width: 100px;transform: scale(0.8)">
                                        <span type="input-group-text button" class="btn btn-danger btn-number btn-sm" data-field="minus">
                                            <span class="fa fa-minus"></span>
                                        </span>
                                        <input type="number" class="form-control form-control-sm bare text-center" data-field='quantity' name="items[{{ $n }}][quantity]" value="0" @if ($retreat_booking->status == 'SOLD') disabled @endif />
                                        <span type="input-group-text button" class=" btn btn-danger btn-number btn-sm" data-field="plus">
                                            <span class="fa fa-plus"></span>
                                        </span>
                                    </div>
                                </td>
                                <td class="fs-6 align-middle pe-0 text-nowrap text-end d-sm-table-cell d-none" width="1%" style="min-width: 60px" data-field="total-price">
                                    @if ($retreat_booking->status == 'SOLD')
            ...
            <div class="col-md-auto">
                <div class="h-100 p-4">
                    <table class="table fs-6" style="min-width: 250px">
                        ...
                        <tr class="discount-field d-none">
                            <td class="ps-0 border-bottom-0 ">Discount Applied</td>
                            <td class="pe-0 border-bottom-0 " id="discount-total">- @money(0)</td>
                        </tr>

                        <tr class="discount-field d-none">
                            <td class="ps-0 border-bottom-0 ">Net Total</td>
                            <td class="pe-0 border-bottom-0 " id="net-total">@money(0)</td>
                        </tr>
                    ...
                    <div>
                        ...
                        <div id="discount-container" class="input-group mb-3 d-none">
                            ...
                            <span type="input-group-text button" class=" btn btn-danger"> Redeem </span>
                        </div>
                        {{-- @guest
                            <div>
                                <hr />
                ...
                <div>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" placeholder="Phone" name="mobile" class="form-control" onkeyup="retreat.target.value = retreat.target.value.replace(/[^\d.-]+/g, '')" />
                    </div>
    ...
    <button type="button" class="btn btn-sm btn-link position-absolute top-0 end-0 " aria-label="Close" data-bs-dismiss="modal"><i class="fas fa-close"></i></button>
</form>
    ...
    $(function() {
        ...
        document.querySelectorAll("[data-row=ticket]").forEach(_row => {
            ...
            _minus_btn.addEventListener("click", () => {
                ...
                var retreat = new Event('change');
                _quantity_field.dispatchEvent(retreat);
            });
